...
using KuCoinConsole.Converters;
...
namespace KuCoinConsole
{
    ...
    public class CryptoCredentials : MemoryEncryptedCredentialsProvider, ICloneable
    {
        ...
        {
            get => string.IsNullOrEmpty(pin) ? pin : AesOperation.DecryptString(Program.Seed, pin);
            set
            {
                if (ValidatePin(value))
                {
                    pin = AesOperation.EncryptString(Program.Seed, value, out _);
                }
            }
        }
